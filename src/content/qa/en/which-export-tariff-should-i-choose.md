...
seo:
  ogTitle: "Choosing an export tariff — fixed, TOU or dynamic?"
---
...
1. **List your candidates' effective rates**, including bundle conditions (import+export from one supplier often unlocks the best export price — compare the *pair*, not the export rate alone).
2. **No battery: take the best flat/fixed export offer** and focus on self-consumption instead — a shifted kWh beats even a good export rate (see the high-bill answer).
3. **With a battery: model one real week** — your export profile against each tariff's price curve; spot-linked export usually wins if automation times the discharge, fixed wins if you want zero involvement.
4. **Re-shop yearly** — this corner of the market moves fast, and switching export supplier is usually low-friction paperwork.
